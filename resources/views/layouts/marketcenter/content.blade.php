...
function get_image($size = 'large') {
  ...
  if(empty($images)){
    for($i = 1; $i<=12; $i++) {
		$images[] =  URL::to('bower_components/realocation/assets/img/tmp/properties/' . $size . '/' . $i . '.jpg'); 
...
function get_agent_image($size = 'medium') {
  ...
  if(empty($agent_images)){
    for($i = 1; $i<=4; $i++) {
		$agent_images[] =  URL::to('bower_components/realocation/assets/img/tmp/agents/' . $size . '/' . $i . '.jpg'); 
